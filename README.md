![docker image](https://github.com/mdinkova-fourco/nomad-distro-template-local-local/actions/workflows/docker-publish.yml/badge.svg)


# mdinkova-fourco's NOMAD Oasis Distribution

This is the NOMAD Oasis distribution of mdinkova-fourco.
...
2. Clone the repository or download the repository as a zip file.
    ...
    ```sh
    git clone https://github.com/mdinkova-fourco/nomad-distro-template-local-local.git
    cd nomad-distro-template-local
    ```

    or

    ```sh
    curl-L -o nomad-distro-template-local.zip "https://github.com/mdinkova-fourco/nomad-distro-template-local-local/archive/main.zip"
    unzip nomad-distro-template-local.zip
    cd nomad-distro-template-local
    ```
...
your `docker-compose.yaml` with `ghcr.io/mdinkova-fourco/nomad-distro-template-local-local:main` for the services
...
and advanced configuration, see the [nomad-helm-charts](https://github.com/mdinkova-fourco/nomad-helm-charts) repository.
...
By default, no plugins are included in this distribution. You can find a list of available NOMAD plugins [here](https://nomad-lab.eu/prod/v1/oasis/gui/search/plugins). For a list of official plugins provided by FAIRmat, please see [here](https://github.com/mdinkova-fourco/.github/blob/main/profile/README.md). For inspiration, you can also check the list of [plugins that are installed on the production NOMAD deployment hosted by FAIRmat](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-distro/-/raw/main/pyproject.toml?ref_type=heads).
...
```sh
docker pull ghcr.io/mdinkova-fourco/nomad-distro-template-local-local/jupyter:main
```
...
By default, documentation is built using the [nomad-docs](https://github.com/mdinkova-fourco/nomad-docs) repository. However, if you'd like to customize the documentation for your Oasis instance, you can easily do so.

1. First, [fork the nomad-docs repository](https://github.com/mdinkova-fourco/nomad-docs/fork).
...
```sh
git remote add template https://github.com/FAIRmat-NFDI/nomad-distro-template-local
git fetch template
...
For detailed instructions on how to resolve the merge conflicts between different version we refer you to the latest template release [notes](https://github.com/mdinkova-fourco/nomad-distro-template-local-local/releases/latest)
...
_I get an_ `Error response from daemon: Head "https://ghcr.io/v2/mdinkova-fourco/nomad-distro-template-local-local/manifests/main": unauthorized`
